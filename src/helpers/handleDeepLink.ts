...
export const handleDeepLink = (link: any) => {
  // todo: Add export type Link to react-native-modules/packages/dynamic-links/src/useDynamicLinks.ts @ line 6
  if (link?.url) {
    ...
    switch (parsedParams.link) {
      case "confirmEmail":
        const confirmEmailParams: ConfirmEmailParams =
          parsedParams as ConfirmEmailParams;
        navigationRef.navigate("ConfirmEmail", { ...confirmEmailParams });
        break;
      case "uploadPictures":
        const takePictureParams: TakePicturesParams =
          parsedParams as TakePicturesParams;
